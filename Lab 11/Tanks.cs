...
while (Tanks.Contains(Player) && Tanks.Count > 1)
{
	...
	foreach (var tank in Tanks)
	{
		...
		if (tank.IsShooting)
		{
			...
			bool collision = BulletCollisionCheck(startbullet, out Tank collisionTank);
			if (!collision)
			{
				tank.Bullet = startbullet;
			}
	...
	foreach (var tank in AllTanks)
	{
		if (tank.Bullet is not null)
		{
			...
			if (bullet.X >= 0 && bullet.X < Console.BufferWidth && bullet.Y >= 0 && bullet.Y < Console.BufferHeight)
			{
